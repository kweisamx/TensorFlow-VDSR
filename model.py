...
class VDSR(object):
    ...
    def build_model(self):
        ...
        self.weights = {
            'w1': tf.Variable(tf.random_normal([3, 3, self.c_dim, 64], stddev=1e-3), name='w1'),
            'w2': tf.Variable(tf.random_normal([3, 3, 64, 64], stddev=1e-3), name='w2'),
            'w3': tf.Variable(tf.random_normal([3, 3, 64, self.c_dim], stddev=1e-3), name='w3')
        }
    ...
    def load(self, checkpoint_dir):
        ...
        """
        print("\nReading Checkpoints.....\n\n")
        model_dir = "%s_%s" % ("vdsr", self.label_size)# give the model name by label_size
    ...
    def save(self, checkpoint_dir, step):
        ...
        model_dir = "%s_%s" % ("vdsr", self.label_size)
        checkpoint_dir = os.path.join(checkpoint_dir, model_dir)
